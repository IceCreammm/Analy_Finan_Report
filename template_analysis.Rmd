...
date_qtr_back <- seq(as.Date(as.character(params$date_report)) + 1,
                   length.out=as.numeric(num_qtr_back) + 1, by='-1 quarter') - 1 
date_qtr_back <- as.character(date_qtr_back)
### get single qtr information
## B/S
df_bal_single <- filter(df_bal_cuml, F001D %in% date_qtr_back) %>% 
  get_increment_from_cumulate("F001D") %>% 
  slice(-1)
## PnL
df_pnl_single <- filter(df_pnl_cuml, F001D %in% date_qtr_back) %>% 
  get_increment_from_cumulate("F001D") %>% 
  slice(-1)
## cash flow
df_cash_single <- filter(df_cash_cuml, F001D %in% date_qtr_back) %>% 
  get_increment_from_cumulate("F001D") %>% 
  slice(-1)
### cute cumulate into the chosen qtr
df_bal_cuml <- filter(df_bal_cuml, F001D %in% df_bal_single$F001D)
df_pnl_cuml <- filter(df_pnl_cuml, F001D %in% df_pnl_single$F001D)
df_cash_cuml <- filter(df_cash_cuml, F001D %in% df_cash_single$F001D)
```
